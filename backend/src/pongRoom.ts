...
export default class PongGame {
	...
	joinGame = (socket: WebSocket, request: FastifyRequest) => {
		let username: string;
		...
		this.sendCmd(currClient, 'ingame', +this.gameState.ingame);
		socket.on('message', (message: any) => this.onMessageHandle(message, currClient));
		socket.on('close', () => {
			console.log(`User ${username} left`);
			if (currClient === this.player2 || currClient === this.player1) {
				this.broadcastCmd("error", `${username} disconnected`);
				this.setWinner(currClient === this.player1 ? this.player2?.username : this.player1?.username)
				return;
			}
			this.clients = this.clients.filter((c: Client) => c.username !== username);
			this.broadcastPosition();
		});
	}
	...
	private changeModeHanble(currClient: Client) {
		if (currClient.registered && currClient === this.player1)
			this.gameState.mode = gameModes[(gameModes.indexOf(this.gameState.mode) + 1) % gameModes.length];
		this.broadcastPlayers();
	...
	private updateGame() {
		...
		if (this.gameState.ballY < this.gameState.ballRadius || this.gameState.ballY > 1000 - this.gameState.ballRadius)
	...
	private playerPaddle() {
		if (
			...
			const pos = (this.gameState.ballY - this.gameState.playerY) / this.gameState.playerHeight;
	...
	private computerPaddle() {
		if (
			...
			const pos = (this.gameState.ballY - this.gameState.computerY) / this.gameState.computerHeight;
	...
	private updatePlayerPaddle(delta: number) {
		...
		this.gameState.playerY = Math.min(this.gameState.playerY, 1000 - this.gameState.playerHeight);
	}
	...
	private updateComputerPaddle(delta: number) {
		...
		this.gameState.computerY = Math.min(this.gameState.computerY, 1000 - this.gameState.computerHeight);
	}

	private checkWinner() {
		if (this.gameState.playerScore < WINNING_SCORE && this.gameState.computerScore < WINNING_SCORE)
			return false;
		this.setWinner(this.gameState.playerScore >= WINNING_SCORE
			? this.player1.username
			: this.player2.username)
		return true;
	}
	...
	private setWinner(winner: string) {
		...
		this.clients.forEach(c => (c.registered = false));
		this.broadcastPosition();
		if (this.player2?.socket) this.clients.push(this.clients.splice(this.clients.indexOf(this.player2), 1)[0]);
		if (this.player2?.socket) this.clients.push(this.clients.splice(this.clients.indexOf(this.player2), 1)[0]);
	}
