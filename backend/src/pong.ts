...
export default function playPong(socket: WebSocket, request: FastifyRequest, app: FastifyInstance) {
	...
	function broadcastPosition() {
		...
		clients.forEach((client: Client) => {
			...
			if (client.socket && client.socket.readyState === 1) client.socket.send(JSON.stringify(obj));
		});
	}
	...
	function checkWinner() {
		if (gameState.playerScore < WINNING_SCORE && gameState.computerScore < WINNING_SCORE) return false;
		broadcastGame();
