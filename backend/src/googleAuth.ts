...
import { createNewUser, getUserBy, setUserBy } from './user';
...
export async function loginUser(googleId: string, reply: FastifyReply) {
    ...
    setUserBy('roomId', 'online', 'googleId', user.googleId);
...
export async function logoutUser(request: FastifyRequest, reply: FastifyReply) {
    ...
    setUserBy('roomId', 'offline', 'googleId', (request.user as any).googleId);
    reply.setCookie('jwt', '', {
        path: '/',
        httpOnly: true,
        sameSite: 'strict',
