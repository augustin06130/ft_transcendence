import { createNewUserGoogle, getUserBy, updateUserDeletionDate } from './user';
...
export async function handleGoogle(request: FastifyRequest, reply: FastifyReply) {
	...
	if (!user) {
		await addUserGoogle(googlePayload.given_name, googlePayload.email, googlePayload.sub);
	}
...
export async function addUserGoogle(
	...
	await createNewUserGoogle(generatedName, email, googleId, deleteDate);
}
