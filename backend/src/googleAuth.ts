...
export async function handleGoogle(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!googlePayload) {
        throw { code: 400, message: 'Undefined google sign-in payload' };
    }
    if (googlePayload.aud !== process.env.GOOGLE_WEB_CLIENT_ID) {
        throw { code: 400, message: 'Invalid audience in token' };
    }

    const user = await getUserBy('googleId', googlePayload.sub);
    if (!user) {
        const id = await addUserGoogle(
