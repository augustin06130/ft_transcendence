...
export default class Tournament {
	...
	stringify(match: Match = this.matches, depth: number = 0, lineUp: Match = this.matches): string {
		let prefix: string = '';
		...
		if (match.left && match.right)
			...
			return `${' '.repeat(depth * 2)}${prefix}${this.getNode(match)}`
	}
