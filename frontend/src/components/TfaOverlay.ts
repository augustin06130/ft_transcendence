...
export default class TfaOverlay {
	...
	constructor(
		...
		this.messageLabel = p({ className: 'mb-6 text-center' }, 'Scan the qrcode with your authentificator application of choice');
		...
		this.buttonConfirm = button({
			onclick: this.sendVerif, className: 'px-3 py-1 border mr-3 border-green-600 rounded hover:bg-green-500/20 transition',
		}, 'CONFIRM');
		this.buttonCancel = button({
			onclick: this.hide, className: 'px-3 py-1 border ml-3 border-green-600 rounded hover:bg-green-500/20 transition',
		}, 'CANCEL');

		this.div = div({ className: 'p-1 m-auto max-w-lg max-h-120 flex flex-col absolute inset-0 rounded items-center justify-center border border-green-500/30 bg-black/90', },
