...
export default class OnlineFriends {
	...
	private makeRow(friendship: FriendShip) {
		...
		const hasRoom = friendship.room !== 'offline' && friendship.room !== 'online';
		return tr(
			{ className: 'h-1' },
			...
			td(
				{
					className: className + (hasRoom ? 'hover:underline' : ''),
	...
	private updateFriends() {
		...
		fetch(url)
			...
			.then(json => {
				if (!json.success)
					throw json.message;
				this.friendsTable.replaceChildren(...json.data.map(this.makeRow));
			})
