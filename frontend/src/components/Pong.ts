...
export default class PongGame {
    ...
    private updateGame(data: Cmd) {
        ...
        this.state.game.speedX = parseFloat(data.arg10) * this.state.canvasWidth;
        this.state.game.speedY = parseFloat(data.arg11) * this.state.canvasHeight;
    ...
    private getNextBall(
        ...
        if (nextY < 0) {
            dtHit = y / Vy;
            return this.getNextBall(x + Vx * dtHit, 0, Vx, -Vy, dt - dtHit);
        }
        if (nextY > this.state.canvasHeight) {
            dtHit = (this.state.canvasHeight - y) / Vy;
            return this.getNextBall(x + Vx * dtHit, this.state.canvasHeight, Vx, -Vy, dt - dtHit);
        }
        ...
        return this.getNextBall(nextX, nextY, Vx, Vy, dt, depth + 1);
    }
