...
function LoginForm(joinHandler: (e: Event) => void, createHandler: (e: Event) => void, roomCode: State<string>) {
	return form({
		className: "space-y-4",
	},
		input({
...
export default function Room() {
	...
	return TerminalBox("terminal@user:~/pong sudo usermod -a -G",
		div({ className: `mx-auto max-w-md border border-green-500/30 rounded p-4 bg-black/80 shadow-lg shadow-green-500/10 transition-opacity duration-500 ${isMounted.get() ? "opacity-100" : "opacity-0"}` },
			div({ className: "text-center mb-6" },
				...
				p({ className: "text-green-400/70 text-sm mt-1" }, "Enter a room to join or create a new room")
			),
			formContent,
