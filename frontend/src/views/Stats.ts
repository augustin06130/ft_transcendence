...
export class Stats {
    ...
    public updateData(data: MatchStatistics) {
        ...
        this.liMatchPlayer.innerText =
            `Match played:`.padEnd(len, ' ') +
            `${Intl.NumberFormat().format(data.countMatch)} matches`;
        this.liWin.innerText =
            `Win:`.padEnd(len, ' ') + `${Intl.NumberFormat().format(data.countWin)} matches`;
        this.liLose.innerText =
            `Loose:`.padEnd(len, ' ') +
            `${Intl.NumberFormat().format(data.countMatch - data.countWin)} matches`;
        this.liAverageScore.innerText =
            `Average score:`.padEnd(len, ' ') + `${data.avgScore.toFixed(2)} points`;
        this.liTotalScore.innerText =
            `Total marked point:`.padEnd(len, ' ') +
        ...
        this.liTotalRally.innerText =
            `Total rallies:`.padEnd(len, ' ') +
            `${Intl.NumberFormat().format(data.sumRally)} rallies`;
        ...
        this.liTotalRallyWin.innerText =
            `Total rallies (win):`.padEnd(len, ' ') +
            `${Intl.NumberFormat().format(data.sumRallyWin)} rallies`;
